...
function init() {
  ...
  $("body").on("click", function (event) {
    if (
      ...
      $(event.target).parents("#intro").length > 0
    ) {
  ...
  if (currSlide.length > 0) {
    ...
    $("span", currSlide).each(function () {
      ...
      if (currIndex < newIndex) {
        var delay =
          ((offsetLeft - minOffset) / (maxOffset - minOffset)) *
          WORD_ANIMATION_SPREAD_TIME;
      } else {
        var delay =
          ((maxOffset - offsetLeft) / (maxOffset - minOffset)) *
          WORD_ANIMATION_SPREAD_TIME;
      }
      setTimeout(function () {
        $this.removeClass("on");
